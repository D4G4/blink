...
///
/// **Recording**: delete the PNG from `__Snapshots__/` and re-run. A new reference is saved
/// to the sandbox container — copy it back to the source tree with:
///   cp ~/Library/Containers/com.blink20.app/Data/Library/Application\ Support/BlinkSnapshots/*.png BlinkSnapshotTests/__Snapshots__/
///
/// **Comparing**: if a reference PNG exists in `__Snapshots__/`, the test renders the view
/// and compares pixel-by-pixel. Mismatches write actual + reference to `__Failures__/`.
class SnapshotTestCase: XCTestCase {
    ...
    /// Writable directory inside the sandbox container for recording new snapshots.
